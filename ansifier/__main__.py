...
"""
This file implements the CLI to ansifier, as well as a handful of utility
functions which make sense in the context of a terminal -
I can see these functions being used in other contexts, however,
so they might become ImageFilePrinter member functions sooner or later.
"""
...
def center_frame_horizontally(frame, width, line_len=None):
    ...
    :param line_len: int, number of characters in a line of output
        post-escaping; pass this in if it's already been calculated,
        otherwise we'll calculate it for you
    :return: frame, modified to be centered based on width
    """
    line_len = length_after_processing(frame.split('\n')[0])
    frame = '\n'.join(
        [' ' * ((width - line_len) // 2) + l\
...
def center_frame_vertically(frame, height, line_len=None, num_lines=None):
    ...
    :param line_len: int, number of characters in a line of output
        post-escaping; pass this in if it's already been calculated,
        otherwise we'll calculate it for you
    :param num_lines: int, number of rows in output; pass this in if
        it's already been calculated, otherwise we'll calculate it for you
    """
    if line_len is None:
        line_len = length_after_processing(frame.split('\n')[0])
    if num_lines is None:
        num_lines = len(frame.split('\n'))
    difference = height - num_lines
    pad_num = difference // 2
    pad_line = ' ' * line_len + '\n'
    pad = pad_line*pad_num
    frame = pad_line + frame
    frame = pad + frame + pad
    return frame
...
def run_cli(args=None):
    """
    :param args: argparse.Namespace, arguments to use;
        should be generated by get_args(), which is what will happen
        if this argument is not specified
    :return: ImageFilePrinter, initialized using args
        ...
        if not args.meofetch:
            if args.animate:
                for i, frame in enumerate(image_printer.frames):
                    image_printer.frames[i] = center_frame_horizontally(
    ...
    if args.center_vertically and height > num_lines:
        if args.animate:
            for i, frame in enumerate(image_printer.frames):
                image_printer.frames[i] = center_frame_vertically(
                    frame, height, line_len, num_lines)
        else:
            image_printer.output = center_frame_vertically(
                image_printer.output, height, line_len, num_lines)
...
def get_parser():
    """
    :return: argparse.ArgumentParser, the default ansifier argument parser
    """
...
def get_args(argparser=None):
    """
    :param argparser: argparse.ArgumentParset, should be retrieved from
        get_parser(), which is what will happen if this argument
        is not set
    :return: argparse.Namespace, result of argparser.parse_args()
    """
